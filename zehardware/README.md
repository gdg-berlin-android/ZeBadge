...
messages (`CONNECT TO WIFI`) and subscribe to specific topics (`Was key 0x63 pressed on keyboard`
or `Did we connect to WiFi?` or `Was up system button pressed?`).
...
Important: Save stores the current configuration on ZeBadge, Update only updates the configuration at runtime, doesn't
store it to the file system. Load loads the stored configuration from the filesystem of the badge.
